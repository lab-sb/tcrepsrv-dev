...
subtitle "Removing TCapture logical decoding library.. "
if [ ! -f /usr/pgsql-10/lib/rdblogdec.so  ]; then 
	 message "rdblogdec.so not exists under /usr/pgsql-10/lib"
else
	subtitle " Remove TCapture logical decoding library under /usr/pgsql-10/lib"
	rm /usr/pgsql-10/lib/rdblogdec.so
fi

subtitle "Removing logs.. "
cd $RDBBDR
rm log/*.log
rm src/TCapt*.log

subtitle "Removing confs.. "
rm conf/*.conf
